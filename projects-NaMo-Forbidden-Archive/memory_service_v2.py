...
class MemoryServiceV2:
    def __init__(self, use_gcs: bool = False, bucket_name: Optional[str] = None):
        ...
        if self.use_gcs:
            if not self.bucket_name:
                print("Warning: USE_GCS is True, but GCS_BUCKET_NAME is not set. Falling back to local storage.")
                self.use_gcs = False
            else:
                try:
                    ...
                    print(f"Warning: Failed to initialize Google Cloud Storage Client: {e}. Falling back to local storage.")
                    self.use_gcs = False
    ...
    async def save_interaction(self, session_id: str, user_message: str, response_data: Dict[str, Any]):
        """
        ...
        state["relationship_stage"] = response_data.get("relationship_stage", state["relationship_stage"])
