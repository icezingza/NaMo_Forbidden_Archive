...
from core.slowburn_lorebook import DEFAULT_LOREBOOK_PATH, SlowBurnLorebook
...
def test_slowburn_lorebook_no_trigger_legacy_single_source():
    # Default multi-source runtime now includes constant Story Engine entries by design.
    # Preserve the legacy no-trigger assertion against the legacy source explicitly.
    lorebook = SlowBurnLorebook(json_path=DEFAULT_LOREBOOK_PATH)
...
def test_slowburn_lorebook_group3_mechanics():
    ...
    assert "RESISTANCE" in SlowBurnLorebook.get_push_pull_phase_directive("resistance")
    assert "NEGOTIATION" in SlowBurnLorebook.get_push_pull_phase_directive("negotiation")
    assert "SURRENDER" in SlowBurnLorebook.get_push_pull_phase_directive("surrender")
...
def test_slowburn_lorebook_group4_memory_continuity():
    ...
    assert SlowBurnLorebook.resolve_attachment_style(30.0, 80.0, 2) == "disorganized"
    assert SlowBurnLorebook.resolve_attachment_style(45.0, 50.0, 2) == "anxious"
    assert SlowBurnLorebook.resolve_attachment_style(80.0, 20.0, 6) == "avoidant"
    assert SlowBurnLorebook.resolve_attachment_style(80.0, 50.0, 2) == "secure"
    assert "ANXIOUS" in SlowBurnLorebook.get_attachment_style_directive("anxious")
