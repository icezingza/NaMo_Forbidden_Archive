...
def build_vector_meta_from_lorebook():
    ...
    lorebook_path = Path(
        "slow-burn-thai-erotic-FULL/full-package/Sex_Positions_Kinks_SlowBurn_TH_v10.json"
    )
    ...
    combined = existing_meta + [
        c for c in new_chunks if c["chunk_id"] not in [e.get("chunk_id") for e in existing_meta]
    ]
