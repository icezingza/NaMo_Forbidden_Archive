...
class DialogueManager:
    ...
    def __init__(self, character_file_path):
        ...
        self.moan_library = (
            self.character_data.get("main_mechanics", {}).get("moan_library", {})
        )
    ...
    def _extract_dialogues(self):
        ...
        return (
            self.character_data.get("main_mechanics", {})
            .get("sample_dialogues", {})
        )
    ...
    def get_response(self, intensity_category):
        ...
        if intensity_category == "high":
            possible_dialogues.extend(
                self.dialogue_templates.get("degradation", [])
            )
            possible_dialogues.extend(
                self.dialogue_templates.get("sensory_attack", [])
            )
        elif intensity_category == "medium":
            possible_dialogues.extend(
                self.dialogue_templates.get("cuckolding", [])
            )
        else:  # low
            # Using gentle dialogues from the v1 file as v2 doesn't explicitly
            # have low intensity samples
            gentle_samples = [
                (
                    "(กระซิบ) พี่รู้มั้ย...เวลาพี่สั่นแบบนี้ "
                    "หนูอยากทำให้พี่เสียวกว่านี้..."
                ),
